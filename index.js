document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayJobListings(jobs) {
      ...
      jobs.forEach(job => {
        ...
        jobElement.innerHTML = `
          ...
          <div class="job-tags">
            ${[job.role, job.level, ...job.languages, ...job.tools].map(tag => `<span class="job-tag">${tag}</span>`).join('')}
          </div>
        `;
    ...
    function addFiltering(jobs) {
      ...
      jobListingsContainer.parentElement.insertBefore(filterInput, jobListingsContainer);
    }
  });